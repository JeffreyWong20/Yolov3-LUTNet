#================================================================
#
#   File name   : binarization_utils_pad.py
#   Description : binary_conv with symmetric padding
#
#================================================================
import numpy as np
import pickle
    ...
    def call(self, x,mask=None):
        ...
        if keras.__version__[0]=='2':
            ...
            if self.levels==1 or self.first_layer==True:
                ...
                if self.PADDING == "SAME" and self.k != 1 :
                    # performing Padding == SAME
                    padding = tf.constant([[0,0],[1, 1,], [1, 1],[0,0]])
                    x = tf.pad(x, padding, "SYMMETRIC") 
                    self.out=K.conv2d(x, kernel=self.clamped_w*tf.reshape(self.pruning_mask, [self.k, self.k, self.ch_in, self.nfilters]), padding="valid",strides=self.strides)
                else:
                ...
                if self.LUT==True:
                    ...
                    if self.PADDING == "SAME" and self.k != 1 :
                        # performing Padding == SAME
                        padding = tf.constant([[0,0],[1, 1,], [1, 1],[0,0]])
                        x_expanded = tf.pad(x_expanded, padding, "SYMMETRIC") 
                        self.out=K.conv2d(x_expanded, kernel=self.clamped_w*tf.reshape(self.pruning_mask, [self.k, self.k, self.ch_in, self.nfilters]), padding="valid",strides=self.strides)
                    else:
