...
def load_yolo_weights(model, weights_file):
    ...
    with open(weights_file, 'rb') as wf:
        ...
        for i in range(range1):
            if i > 0:
                conv_layer_name = 'binary_conv_%d' %i
            else:
                conv_layer_name = 'binary_conv'              
            ...
            # filters = conv_layer.get
            # k_size = conv_layer.kernel_size[0]
            # in_dim = conv_layer.input_shape[-1]
            filters = conv_layer.nfilters
            k_size = conv_layer.k
            in_dim = conv_layer.ch_in
            ...
            tmp = conv_weights.reshape(conv_shape).transpose([2, 3, 1, 0])
            init = conv_layer.get_weights()           
            init[0] = tmp 
            if i not in range2:
                #conv_layer.set_weights([conv_weights])
                conv_layer.set_weights(init)
                print("Name: ",conv_layer_name," Shape: ",init[0].shape)
                bn_layer.set_weights(bn_weights)
                print("Name: ",bn_layer_name," Shape: ",bn_weights.shape)
            else:
                #conv_layer.set_weights([conv_weights, conv_bias])
                conv_layer.set_weights(init)
                print("Name: ",conv_layer_name," Shape: ",init[0].shape, " Bias: ", conv_bias.shape)
